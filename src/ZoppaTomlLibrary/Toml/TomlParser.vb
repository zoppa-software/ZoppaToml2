...
#Region "文字列"
        ...
        ''' <summary>
        ''' 引数で指定されたU8Stringのイテレータから、3つのダブルコーテーションが連続しているかをチェックします。
        ''' 3つのダブルコーテーションは複数行基本文字列の開始記号として使用されます。
        ''' </summary>
        ''' <param name="iter">U8Stringのイテレータ。</param>
        ''' <returns>3つのダブルコーテーションが連続している場合は真。</returns>
        Private Function MatchMlBasicStringDelim(iter As U8String.U8StringIterator) As Boolean
            Dim startIndex = iter.CurrentIndex
        ...
        ''' <summary>
        ''' 引数で指定されたU8Stringのイテレータから、3つのアポストロフィが連続しているかをチェックします。
        ''' 3つのアポストロフィは複数行リテラル文字列の開始記号として使用されます。
        ''' </summary>
        ''' <param name="iter">U8Stringのイテレータ。</param>
        ''' <returns>3つのアポストロフィが連続している場合は真。</returns>
        Private Function MatchMlliteralStringDelim(iter As U8String.U8StringIterator) As Boolean
            Dim startIndex = iter.CurrentIndex
        ...
        ''' <summary>
        ''' 複数行基本文字列の中で、アポストロフィが連続しているかをチェックします。
        ''' 連続している場合は、複数行基本文字列の内容として認識されます。
        ''' </summary>
        ''' <param name="iter">U8Stringのイテレータ。</param>
        ''' <returns>アポストロフィが連続している場合は真。</returns>
        Private Function MatchMllQuotes(iter As U8String.U8StringIterator) As Boolean
            Dim startIndex = iter.CurrentIndex
            ...
            ' アポストロフィが連続していない場合はイテレータの位置を元に戻す
            iter.SetCurrentIndex(startIndex)
            Return False
        End Function
...
#Region "数値"
        ...
        Function ConvertToFloat(iter As U8String.U8StringIterator) As Double
            ...
            If iter.HasNext() AndAlso AnyCharByte(iter.Current, &H65, &H45) Then
                ...
                ' 指数部の値を調整
                decfig += If(expSign, -CInt(expValue), CInt(expValue))
            End If
